...
int main(void)
{
    ...
    Val values[N];
    Key key;
    FILE *file = fopen("/home/etryfly/Документы/Labs/2sem/KP9/testData.txt", "r");
    ...
    while (i < N && fscanf(file, "%d", &keys[i].key.real) == 1)
    {
        fscanf(file, "%d", &keys[i].key.im);
        fscanf(file, "%c", &ch);
    ...
    do
    {
        ...
        switch (action)
        {
            ...
            case 2:
            {
                ...
                {
                    printf("Введите ключ: ");
                    scanf("%d %d", &key.key.real, &key.key.im);
...
void printTable(const Key *k, const Val *v, const int size)
{
    ...
    for (i = 0; i < size; i++)
        printf("|%8d %8d|%60s|\n", k[i].key.real, k[i].key.im,  v[i].val);
...
int comparator(const Key k1, const Key k2)
{
    if (k1.key.real > k2.key.real) {
        return 1;
    }
    if (k1.key.real == k2.key.real && k1.key.im > k2.key.im) {
        return 1;
    }
    return 0;
...
int isEqualKeys(const Key k1, const Key k2)
{
    return (k1.key.im == k2.key.im && k1.key.real == k2.key.real);
}
