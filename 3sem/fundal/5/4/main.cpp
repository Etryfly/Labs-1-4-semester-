    ...
    if (a.size < MAX_KARA) {
        ...
        for (int i = 0; i < a.size; ++i)
            for (int j = 0, carry = 0; j < b.size || carry; ++j) {
                cur = product.long_number[i + j] + a.long_number[i] * 1ll * (j < (int) b.size ? b.long_number[j] : 0) +
                      carry;
...
char *my_itoa(LongNumber &number, int base) {
    ...
    while (number != 0) {
        ...
        result[size - 1] = (temp.get_short_number() > 9 ? temp.get_short_number() - 10 + 'A' : temp.get_short_number() +
                                                                                               '0');
        number /= base_long;
        size++;
